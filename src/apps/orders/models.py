...
class Order(models.Model):
    ...
    delivery_type = models.CharField(max_length=50) # e.g., 'pickup', 'courier'
    address_snapshot = models.JSONField() # Stores the address at the time of the order
    payment_method = models.CharField(max_length=50)
...
class OrderItem(models.Model):
    order = models.ForeignKey(Order, on_delete=models.CASCADE, related_name='items')
    product = models.ForeignKey('catalog.Product', on_delete=models.SET_NULL, null=True)
    name_snapshot = models.CharField(max_length=255) # Stores product name at time of purchase
    article_snapshot = models.CharField(max_length=100) # Stores SKU at time of purchase
    price = models.DecimalField(max_digits=12, decimal_places=2) # Stores price at time of purchase
    quantity = models.PositiveIntegerField(default=1)
